    ...
    var circle = paper.circle(getWidth()/2,getHeight()/2,30);
    circle.attr("fill","#159");
    circle.glow();

    var text = paper.text(getWidth()/2,getHeight()/2,"Distance\n"+"No Data");
    text.attr({ "font-size": 16, "font-family": "\"Courier New\", Courier, monospace" });
...
function getWidth(){
    ...
    function createArc(){
        ...
        let tpath=Raphael.transformPath(arc.attr("path"),'T0,-200,S2');
